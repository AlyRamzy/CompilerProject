...
stmt_block:         '{' '}'                     { $$ = new BlockNode($1); }
    |               '{' stmt_list '}'           { $$ = new BlockNode($1, *$2); delete $2; }
    ;

stmt:               ';'                         { $$ = new StatementNode($1); }
    |               BREAK_TOKEN ';'             { $$ = new BreakStmtNode($1); }
    |               CONTINUE_TOKEN ';'          { $$ = new ContinueStmtNode($1); }
    |               expression ';'              { $$ = new ExpressionContainerNode($1->loc, $1); }
    |               var_decl ';'                { $$ = $1; }
    |               if_stmt                     { $$ = $1; }
    |               switch_stmt                 { $$ = $1; }
    |               case_stmt                   { $$ = $1; }
...
expression:         ident '=' expression                    { $$ = new AssignmentOperationNode($2, $1, $3); }
    |               expression '+' expression               { $$ = new BinaryOperationNode($2, ADD, $1, $3); }
    |               expression '-' expression               { $$ = new BinaryOperationNode($2, SUB, $1, $3); }
    |               expression '*' expression               { $$ = new BinaryOperationNode($2, MUL, $1, $3); }
    |               expression '/' expression               { $$ = new BinaryOperationNode($2, DIV, $1, $3); }
    |               expression '%' expression               { $$ = new BinaryOperationNode($2, MOD, $1, $3); }
    |               expression '&' expression               { $$ = new BinaryOperationNode($2, AND, $1, $3); }
    |               expression '|' expression               { $$ = new BinaryOperationNode($2, OR, $1, $3); }
    |               expression '^' expression               { $$ = new BinaryOperationNode($2, XOR, $1, $3); }
    |               expression LOGICAL_AND_TOKEN expression { $$ = new BinaryOperationNode($2, LOGICAL_AND, $1, $3); }
    |               expression LOGICAL_OR_TOKEN expression  { $$ = new BinaryOperationNode($2, LOGICAL_OR, $1, $3); }
    |               expression '>' expression               { $$ = new BinaryOperationNode($2, GT, $1, $3); }
    |               expression GTE_TOKEN expression         { $$ = new BinaryOperationNode($2, GTE, $1, $3); }
    |               expression '<' expression               { $$ = new BinaryOperationNode($2, LT, $1, $3); }
    |               expression LTE_TOKEN expression         { $$ = new BinaryOperationNode($2, LTE, $1, $3); }
    |               expression EQ_TOKEN expression          { $$ = new BinaryOperationNode($2, EQ, $1, $3); }
    |               expression NEQ_TOKEN expression         { $$ = new BinaryOperationNode($2, NEQ, $1, $3); }
    |               '+' expression %prec U_PLUS_TOKEN       { $$ = new UnaryOperationNode($1, U_PLUS, $2); }
    |               '-' expression %prec U_MINUS_TOKEN      { $$ = new UnaryOperationNode($1, U_MINUS, $2); }
    |               '~' expression                          { $$ = new UnaryOperationNode($1, NOT, $2); }
    |               '!' expression                          { $$ = new UnaryOperationNode($1, LOGICAL_NOT, $2); }
    |               '(' expression ')'                      { $$ = new ExpressionContainerNode($1, $2); }
    |               value                                   { $$ = $1; }
    |               ident                                   { $$ = $1; }
    |               function_call                           { $$ = $1; }
    ;
